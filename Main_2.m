...
for i = 1:length(worldCoordsNodes(num).Nodes)
    ...
    Poses(i).Pose = poses;
    % Sometimes a pose returns a weird value
end
...
[P, skeleton_coords, nodes] = ...
    get_vernoli_path(sceny, 0, 0); % TODO currently this function uses two
    % points which I defined inside the function. Instead of 0, 0, it
    % should take the starting and ending coords and be able to find the 
    % closest entry and exit to the path.

% Extract red path from output P
path = (P(:,:,1) == 255) & ~P(:,:,2) & ~P(:,:,3);

% Show original scene with Vernoli nodes on it and red path
scene2D = imoverlay(scene2D, path, 'red');
imshow(scene2D);
hold on;

% Plot start and end points (need to go from these points to
% the red line path. For now I just added these to array "nodes").
plot(startXY(1), startXY(2), 'g*', 'MarkerSize', 15);
plot(endXY(1), endXY(2), 'g*', 'MarkerSize', 15)

% Preprends starting coords and appends ending coords to nodes.
nodes = [startXY; nodes];
nodes = [nodes; endXY];

% Plot main nodes - this is what the robot will aim for. The step size can
% be changed inside get_vernoli_path
plot(nodes(:,1), nodes(:,2), 'cd', 'MarkerSize', 10);
